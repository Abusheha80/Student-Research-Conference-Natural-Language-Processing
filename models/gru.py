...
# GRU model
def create_gru_model(vocab_size=MAX_VOCAB_SIZE, embed_dim=100, input_len=MAX_SEQUENCE_LEN):
    model = Sequential()
    model.add(Embedding(input_dim=vocab_size, output_dim=embed_dim, input_length=input_len))
    model.add(GRU(128))
...
plt.tight_layout()
plt.savefig(f'{outdir}/gru_confusion_matrix.png')
plt.close()
...
plt.legend()
plt.savefig(f'{outdir}/gru_roc_curve.png')
plt.close()
...
df_metrics.to_csv(f'{outdir}/gru_metrics.csv', index=False)
